...
/**
    *@brief Serial port protocol analysis
        ...
        for(int i = 0; i < readed_len; ++i) {
            ...
            switch(self->state) {
                ...
                case PACKET_CONTROLLER_STATE_DATA:/*Process frame data */
                    ...
                    break;
                case PACKET_CONTROLLER_STATE_CHECKSUM:/*Process check value */
                    ...
                    printf("crc:%d\r\n",crc);
                    if(crc == self->frame.data_and_checksum[self->frame.data_length]) {/*Check failed, execution skipped */
